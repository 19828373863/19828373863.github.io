...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const defaultEncoding = translate
    .defaultEncoding // 網站默認語言，1: 繁體中文, 2: 簡體中文
  const translateDelay = translate
    .translateDelay // 延遲時間,若不在前, 要設定延遲翻譯時間, 如100表示100ms,默認為0
  const msgToTraditionalChinese = translate
    .msgToTraditionalChinese // 此處可以更改為你想要顯示的文字
  const msgToSimplifiedChinese = translate
    .msgToSimplifiedChinese // 同上，但兩處均不建議更改
  ...
  function translateBody(fobj) {
    ...
    for (let i = 0; i < objs.length; i++) {
      ...
      if (obj.alt !== '' && obj.alt != null) obj.alt = translateText(obj
        .alt)
      if (obj.placeholder !== '' && obj.placeholder != null) obj
        .placeholder = translateText(obj.placeholder)
      if (
        obj.tagName === 'INPUT' &&
        obj.value !== '' &&
        obj.type !== 'text' &&
  ...
  function translateInitialization() {
    ...
    if (translateButtonObject) {
      if (currentEncoding !== targetEncoding) {
        ...
        if (targetEncoding === 1) translateButtonObject.innerHTML =
